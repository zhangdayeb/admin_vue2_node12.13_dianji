...
<script>
  import {getArticleTypeListApi, getArticleTypeDelApi,getArticleTypeEditApi,getArticleTypeAddApi} from '@/api/ArticleTypeApi'
  export default {
    data () {
      return {
        allpage:1,//总页数
    ...
    methods: {
      ...
      onSubmit() {
        this.dvEdit = false
        if(this.form.id){
          getArticleTypeEditApi(this.form).then(res => {
          })
          return;
        }
         getArticleTypeAddApi(this.form).then(res => {
            this.getUserList()
         })
      },
